...
def wired(monkeypatch):
    ...
    calls = SimpleNamespace(withdraw=[], tenants=[], deletions=[], withdrawn=())
    ...
    async def fake_withdraw(user_id, scopes, ip, ua, db):
        ...
        return Withdrawal(record=ConsentRecord(user_id=user_id), withdrawn=calls.withdrawn)
    ...
    async def fake_execute_deletion(user, mode, db):
        ...
        return {"mode": mode.value, "hard_delete_after": "2026-10-25T00:00:00+00:00"}
...
def test_withdrawal_is_recorded_with_the_request_details(wired):
    wired.withdrawn = ("llm_processing",)
...
def test_withdrawing_data_processing_schedules_account_deletion(wired):
    wired.withdrawn = ("data_processing",)
    outcome = _withdraw(["data_processing"])

    assert wired.deletions == [(USER, DeletionMode.soft_delete, SHARED_DB)]
    assert outcome.account_deletion == {
        "mode": "soft_delete", "hard_delete_after": "2026-10-25T00:00:00+00:00",
    }


def test_a_repeated_withdrawal_triggers_nothing_again(wired):
    wired.withdrawn = ()
    outcome = _withdraw(["auto_apply", "data_processing"])

    assert outcome.withdrawal.withdrawn == () and outcome.account_deletion is None
    assert wired.tenants == [] and wired.deletions == []
