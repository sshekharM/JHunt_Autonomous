...
Python FastAPI job hunt automation, brownfield existing code, web UI + API
...
**Import hierarchy:** not enforced (gate disabled; see Architecture below — `.claude/architecture.md` is the generic template and does not describe this repo).
**Sensor tier:** `standard` — the pre-commit gate set enforced in this project.
**Topology:** api-service · **agent model tier:** cost.
...
Single-root repo (no `backend/` or `frontend/` dirs). Dependencies: `pip install -r requirements.txt` into a venv.

**Tests:** `pytest -x -q` (config in `pytest.ini`, `testpaths = tests`)
**Lint / types:** `ruff check --fix .` | `mypy app/`
**Migrations:** `alembic upgrade head`
**Full stack:** `docker compose up -d --build` (services: db, redis, minio, app, worker, beat)
**Health:** `GET http://localhost:8000/api/health`

## Architecture

FastAPI monolith under `app/`, not the generic 6-layer template — the layer-import gate is disabled (`project-manifest.json#architecture.enabled: false`).

- `app/routers/` → `app/services/` → `app/models/` / `app/tenant_models/` (routers stay thin; logic lives in services)
- `app/schemas/` holds Pydantic request/response models
- `app/tasks/` Celery tasks; `app/crawlers/` Playwright portal crawlers; `app/ml/`, `app/llm/` matching
- **Schema-per-tenant:** each user gets PostgreSQL schema `u_<thumbprint[:32]>`. Shared tables in `app/models/`, per-user tables in `app/tenant_models/`. Never interpolate an unvalidated schema name into SQL.

Map, risks, and glossary: `specs/brownfield/wiki/WIKI.md`, `specs/brownfield/risk-map.md`, `specs/design/CONTEXT.md`.
...
- Functions ≤ 30 lines, files ≤ 300 lines (tests 500) — enforced by pre-write-gate as a ratchet: existing oversized code is grandfathered, but an edit may not make it worse or push new code over the cap
